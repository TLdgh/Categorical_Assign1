...
pchisq(G_value, df=6, lower.tail=FALSE)
...
pchisq(G_value, df=3, lower.tail=FALSE)
```
...
```{r}
Vig_Rec<-nhanes%>%transmute(InRange=factor((ifelse(bmi>18.5 & bmi <30, "Yes", "No")), levels = c("Yes", "No")), vigrecexr)%>%na.omit()%>%filter(vigrecexr=="Yes")
Vig_Rec%>%group_by(InRange)%>%count()
p1<-1028/(1028+371)

Mod_Rec<-nhanes%>%transmute(InRange=factor((ifelse(bmi>18.5 & bmi <30, "Yes", "No")), levels = c("Yes", "No")), modrecexr)%>%na.omit()%>%filter(modrecexr=="Yes")
Mod_Rec%>%group_by(InRange)%>%count()
p2<-1657/(1657+828)
z<-(p1-p2)/sqrt(p1*(1-p1)/(1028+371) + p2*(1-p2)/(1657+828))
pnorm(-z) #reject null


Vig_Rec<-nhanes%>%transmute(InRange=factor((ifelse(bmi>18.5 & bmi <30, "Yes", "No")), levels = c("Yes", "No")), vigrecexr)%>%na.omit()%>%filter(vigrecexr=="No")
Vig_Rec%>%group_by(InRange)%>%count()
p1<-2981/(2981+2064)

Mod_Rec<-nhanes%>%transmute(InRange=factor((ifelse(bmi>18.5 & bmi <30, "Yes", "No")), levels = c("Yes", "No")), modrecexr)%>%na.omit()%>%filter(modrecexr=="No")
Mod_Rec%>%group_by(InRange)%>%count()
p2<-2351/(2351+1607)
z<-(p1-p2)/sqrt(p1*(1-p1)/(2981+2064) + p2*(1-p2)/(2351+1607))
2*pnorm(z) #reject null
```
...
nhanes_data$bp_category <- with(nhanes_data, ifelse(
  ...
  ifelse(sysbp >= 140 | dbp >= 90, "Hypertension Stage 2", NA))))))
...
pvals1 <- test1$pval_df %>%
  kable(caption = "p-values of Independence Tests between Weight and Cholesterol") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), position = "float_left")

pvals2 <- test2$pval_df %>%
  kable(caption = "p-values of Independence Tests between Weight and Blood Pressure") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), position = "float_left")

pvals3 <- test3$pval_df %>%
  kable(caption = "p-values of Independence Tests between Cholesterol and Blood Pressure") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), position = "float_left")
