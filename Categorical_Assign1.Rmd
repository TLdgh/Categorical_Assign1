...
In order to find the relationship between weight and gender or age , we first categorize the numerical variables. As mentioned in Table 1, we use the BMI level to categorize the weight and we still use this standard in the following process. For age, we are divided into three groups based on NIH recommendations: younger than 18 for adolescents, 18 to 65 for adults, and older than 65 for older adults. Then we crate the contingency tables for variables, and use pearson GOF chi-square test and likelihood ratio test to testing independence and discuss difference from two test methods.
...
tableBA<-testBA$ContingencyTbl$BMI_level%>%
  ...
  kable(caption = "Contingency Table for BMI level and Agegroup")%>%
...
tableBG<-testBG$ContingencyTbl$BMI_level%>%
  rename("\t" =BMI_level) %>%
  add_column(" " = c("BMI level", rep(" ", nrow(.) - 1)), .before = "\t") %>%
  kable(caption = "Contingency Table for BMI level and Gender")%>%
  kable_styling(bootstrap_options=c("striped","condensed"),position="float_left")%>%
...
testBA$pval_df%>%
  kable(caption = "p-values of Independence Tests between Age group and BMI_level")%>%
  kable_styling(bootstrap_options=c("striped","condensed"),position="float_left")

#calculate the p-value
testBG$pval_df%>%
  kable(caption = "p-values of Independence Tests between Age group and Gender")%>%
  kable_styling(bootstrap_options=c("striped","condensed"),position="float_left")
...
PA=pchisq(G_value, df=6, lower.tail=FALSE)
...
PG=pchisq(G_value, df=3, lower.tail=FALSE)
...
```
First, we ascertain the p-value by Chi-square test, the p-valve for weight and age is$2.543\times10^{-32}$ and p-value for weight and gender is $6.311\times10^{-13}$.The p-values from likelihood ratio test are slightly different with Chi-square test, $1.976\times10^{-29}$ for weight and age, $5.223\times10^{-13}$ for weight and gender. Both two methods have the p-value much less than 0.001, which indicates that therPA exist relationship between weight and age , weight and gender. Hence we rejected the null hypothesis and considered there exist sighificant relationship. 
