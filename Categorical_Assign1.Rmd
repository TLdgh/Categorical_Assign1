...
table_weight_chol <- test1$res$weight_category %>%
...
table_weight_bp <- test2$res$weight_category %>%
  rename("\t" = weight_category) %>%
  add_column(" " = c("Weight", rep(" ", nrow(.) - 1)), .before = "\t") %>%
...
table_chol_bp <- test3$res$chol_category %>%
