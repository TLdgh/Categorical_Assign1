...
From the table 1, we can realized that there are 6482 observations with 21 variables in our data set and 8 variables can be considered as categorical variables. This data set mainly focus on the observers between 16 to 80 years old. Among them, the number of 80-year-old is the largest among all ages, and as we can see from the bar chart that the number of men and women is similar.

```{r}
#exploratory age and gender 
gender<-nhanes%>%
  group_by(gender,age)%>%
  summarise(count=n())
   ggplot(gender,aes(x=age, y=count), group=gender)+
geom_bar(stat='identity',aes(fill=gender),position = position_dodge(0.7),width=0.7)+
  scale_y_continuous(
    labels = scales::comma
  ) +
    theme(axis.text.x = element_text(size =7, vjust = 0.5, hjust = 0.5, angle = 45))+
   scale_fill_manual(values=c("blue","red"))+
   labs(title="Number of Observers in Different Age")+
   theme(plot.title = element_text(hjust = 0.5))
```

For the observations in different marital status, we also interested in the relationship with weight.The following box plot shows that the average weight under different marital status are all around 80 Kg.
