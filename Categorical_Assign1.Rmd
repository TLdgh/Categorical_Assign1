...
library(tableone)
#variables
listvar<-c("gender","age","marstat","samplewt","psu","strata","tchol","hdl","sysbp","dbp","wt","ht","bmi","vigwrk","modwrk","wlkbik","vigrecexr","modrecexr","sedmin","obese")
catvar<- c("gender","marstat","vigwrk","modwrk","wlkbik","vigrecexr","modrecexr","obese")
#table
table1<-CreateTableOne(vars=listvar, data=nhanes, factorVars=catvar)
table1
```
...
Catenhanes<-nhanes%>%
  mutate(BMI_level = case_when(
    ...
    nhanes$bmi < 35 ~"Class1 Obesity ",
    nhanes$bmi < 40 ~"Class2 Obesity ",
    nhanes$bmi >= 40 ~"Server Obesity"),
   group=case_when(
