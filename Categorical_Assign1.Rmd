...
IndepTest<-setRefClass(
  "IndepTest",
  fields = list(data="data.frame", variable_x="character", variable_y="character",res="data.frame"),
  methods=list(
    initialize=function(data, variable_x, variable_y){
      #make a contingency table for the variable X and Y
      .self$res<-data%>%select(all_of(c(variable_x, variable_y)))%>%na.omit()%>%
    ...
    get_pvalue=function(){
      ...
      test<-ungroup(res)%>%select(-any_of(group_vars(res)))%>%as.matrix()%>%chisq.test()
      return(test$p.value)}
  )
)
```
...
```{r}
test1<-IndepTest$new(nhanes, "marstat", "obese")
pv<-test1$get_pvalue()
```
